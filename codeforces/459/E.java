...
public class cf5 implements Runnable{    
	...
	public void run() {
		...
		ArrayList<pair>[] edge = new ArrayList[(int)1e5 + 1];
		...
		for(int i = 0; i < m; i++) {
			...
			int u = s.nextInt(), v = s.nextInt(), wt = s.nextInt();
			edge[wt].add(new pair(u, v));
		}
		...
		for(int i = 1; i <= (int)1e5; i++) {
			...
			for(int j = 0; j < edge[i].size(); j++) {
				...
				if(dp[u] + 1 > dp[v] && (hm.get(v) == null || dp[u] + 1 > hm.get(v))) 
					hm.put(v, dp[u] + 1);
			}
